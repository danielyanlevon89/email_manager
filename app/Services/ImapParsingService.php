...
class ImapParsingService
{
    ...
    {
        $commonSettings = (new CommonSettingController)->getCommonSettings($account);
        ...
        try {
            ...
            if ($client->isConnected()) {
                ...
                foreach ($this->folders as $folder) {
                    ...
                    if ($folder == 'INBOX') {
                        $this->addIncomingEmails($account, $client, $commonSettings);
                    } elseif ($folder == 'Sent') {
    ...
    private function addIncomingEmails(EmailAccount $account, $client, $commonSettings)
    {
        ...
        $query
            ...
            ->limit(limit: 10)
            ->fetchOrderDesc()->chunked(function ($messages, $chunk) use ($account, $commonSettings) {
                ;
                $messages->each(function ($message, $key) use ($account, $commonSettings) {
                    ...
                    if (IncomingEmail::where([
                        ...
                        $this->incomingEmails['subject'] = $message->getAttributes()["subject"];
                        ...
                        try {
                            ...
                            $containBlackListWord = (new BlackListService())->checkContentContainBlackListWord($account, $this->incomingEmails['subject'], $this->incomingEmails['body']);

                            if ($account->auto_reply_is_active && $conversationEmailsCount == 1 && !$containBlackListWord) {
                                Str::of($this->incomingEmails['from'])->explode(',')->each(function (string $item, int $key) use ($account, $message, $commonSettings) {
                                    ...
                                    $emailData = [
                                        'mail_to' => $item,
                                        'mail_to_name' => Str::getEmailNameFromString($message->getAttributes()['fromaddress']),
                                    ...
                                    SendMail::dispatch(
                                        $emailData,
                                        $account->id
                                    )->delay(now()->addMinutes($commonSettings->auto_reply_delay ?? 0));;
            ...
            ->chunked(function ($messages, $chunk) use ($account) {
                ...
                $messages->each(function ($message, $key) use ($account) {
                    ...
                    if (OutgoingEmail::where([
                        ...
                        $this->outgoingEmails['subject'] = $message->getAttributes()["subject"];
